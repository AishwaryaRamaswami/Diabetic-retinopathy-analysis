...
##        self.listOfWinFrame.append(seventhFrame)
        ...
        ### Welcome Frame ###
        WindowLabel = Label(self.FirstFrame.getWindowFrame(), text = "Feature Extraction", height = 1, width = 40)
        WindowLabel.place(x=150, y=30)
        WindowLabel.configure(background="White", font=("Times New Roman", 16, "bold"))

        # Adding RadioButtons to Welcome Frame
        self.val = IntVar()
        RB1 = Radiobutton(self.FirstFrame.getWindowFrame(), text="Extraction of Exudates", variable=self.val, value=1, command=self.check)
        RB1.place(x=100,y=100)
        RB2 = Radiobutton(self.FirstFrame.getWindowFrame(), text="Extraction of Blood Vessels", variable=self.val, value=2, command=self.check)
        RB2.place(x=100,y=120)
        RB3 = Radiobutton(self.FirstFrame.getWindowFrame(), text="Extraction of Microaneurysms", variable=self.val, value=3, command=self.check)
        RB3.place(x=100,y=140)

        # Adding Browse Button to Welcome Frame
        browseBtn = Button(self.FirstFrame.getWindowFrame(), text="Browse", width=8, command=self.browseWindow)
...
### Browse Button for Welcome Frame ###
    def browseWindow(self):
        ...
        self.listOfWinFrame[0].setImage(imgRes)     ##Passing Image to Frame for setting
        self.listOfWinFrame[0].displayImage()       ##Displaying Image
        self.ExEd.setImage(imgRes)                  ##Passing Image to Exudates for Processing
        self.BV.setImage(imgRes)
